    ...
    for (let e of form) {
      ...
      if (question.text == "") {
        this.toasterService.addWarnMessage(
            'Une question doit avoir un titre',
        );
        return false;
      }
      if (question.offeredAnswers.length == 0) {
        this.toasterService.addWarnMessage(
            'une question doit avoir au moins un choix à sélectionner',
        );
        return false;
      }
      if (question.selectedAnswers == "") {
        this.toasterService.addWarnMessage(
            'une question doit avoir au moins une réponse correcte selectionné',
        );
        return false;
      }
      ...
      for (let option of question.offeredAnswers) {
        ...
        if (option.value == "") {
            this.toasterService.addWarnMessage(
                'une option doit avoir un texte',
            );
          return false;
        }
      }
